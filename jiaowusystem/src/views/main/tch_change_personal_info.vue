...
export default {  
  ...
  methods: {  
    fetchUserInfo() {
      ...
      axios.get(url)
        .then(response => {
          ...
          this.change_info.user_status = data.tch_status;
    ...
    saveInfo() {  
      ...
      const tchData = {
      tch_id: this.change_info.user_id,
      tch_name: this.change_info.name,
      tch_sex: this.change_info.sex,
      tch_sch: this.change_info.college,
      tch_tel: this.change_info.tel,
      tch_picture: this.change_info.avatar, // 假设前端上传图片后存储的相对路径或URL
      tch_account: this.change_info.user_account,
      tch_password: this.change_info.password,
      tch_status: this.change_info.user_status,
    };
